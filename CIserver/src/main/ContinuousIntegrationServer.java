...
public class ContinuousIntegrationServer extends AbstractHandler {
            ...
            throws IOException, ServletException
    {
        ...
        if (request.toString().contains("POST")) {
            ...
            System.out.println("raw JSON: " + tmp_str);
            jsonParser(tmp_str);
            System.out.println("\nNew commit detected at " + name + "!");
            getProjectFromGit(clone_url, branch, buildpath);
            System.out.println("- - - - - building " + name + " - - - - -");
            build(buildpath);
            System.out.println("- - - - - testing " + name + " - - - - -");
            test();
            writeToFile("history/" + email + "_" + time + ".txt");

            email = "budithi2@illinois.edu";

            String y = outputFromCI.toString();
            System.out.println("\nThe following output will be sent to " + email + ":\n\n");
    ...
    public void build(String souceDIR){
        System.out.println("\nProject will be built in directory " + buildpath + "\n");
        ...
        outputFromCI.append("Most recent commit by " + email + " at " + branch + " branch of " + name + "\n\n");
        outputFromCI.append("BUILD STATUS:\n\n");
        for(File javaFile: javaFiles){
            ...
            if(compileResult == 0){
                outputFromCI.append(javaFile.getName() + " build successful \n");
                outputFromCI.append(output.toString());
            }else {
                outputFromCI.append(javaFile.getName() + " build failed \n");
                outputFromCI.append(output.toString());
                ...
                if(javaFile.getName().toLowerCase().contains("test")){
                    ...
                    try{
                        ...
                        if (pro.exitValue() == 0) {
                            ...
                            outputFromCI.append("- - - Test Passed - - -\n");
                        } else {
                            System.out.println("- - - Test Failed - - -");
                            outputFromCI.append("- - - Test Failed - - -\n");
                        }
...
//                        outputFromCI.append("exitValue() " + pro.exitValue() + "\n");
            ...
            System.out.println("Build failed, no tests were run");
    ...
    public void jsonParser(String str){
        ...
        name = obj.getJSONObject("repository").getString("full_name");
        ...
        time = obj.getJSONObject("repository").getString("updated_at");
    }
...
//        }
        ...
        writer.print(outputFromCI.toString());
        writer.close();
